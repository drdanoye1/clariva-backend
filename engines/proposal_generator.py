...
from models.schemas import StructuredSectionContent, ParagraphBlock, CalloutBlock
from utils.block_renderer import flatten_section_blocks_to_text
...
{"schemaVersion": "1.0", "blocks": [ ... ]}

Each entry in "blocks" is one of these block types — use whichever fits the content, do not
force everything into "paragraph":

- {"type": "paragraph", "text": "..."} — a normal prose paragraph. If a paragraph has a genuine
  information gap, add "missing": {"field": "<short_snake_case_key>", "label": "<human label of
  what's missing>", "severity": "blocking"} instead of writing "[MISSING: ...]" inside the text.
- {"type": "runIn", "label": "Personnel:", "text": "..."} — bold label + body on one line, for
  Personnel, Equipment, Travel, Indirect Costs and similar labeled sub-items.
- {"type": "bulletList", "items": ["...", "..."]}
- {"type": "numberedList", "items": ["...", "..."]}
- {"type": "table", "caption": "...", "columns": ["...", "..."], "rows": [["...", "..."], ...]}
  — a REAL table (never markdown pipe syntax) any time you'd present tabular, budget, or
  comparison data.
- {"type": "schedule", "title": "Project Schedule", "phases": [{"name": "...",
  "start_month": 1, "end_month": 6}, ...]} — start_month/end_month are 1-indexed inclusive
  integers. Use only for the project timeline/milestones section — see SCHEDULE guidance below.
- {"type": "figure", "caption": "Short figure title", "altText": "Detailed description of what
  this figure should show"} — see FIGURES guidance below. Never set "assetId".
- {"type": "callout", "kind": "missing"|"warning"|"note"|"info", "title": "...", "text": "..."}
  — use kind "missing" for a whole missing sub-topic (not a single detail — use a paragraph's
  "missing" field for that).
- {"type": "pageBreak"}
- {"type": "references", "entries": [{"text": "...", "url": "..."}]}

Rules:
- Do NOT use markdown anywhere inside any string value (no **, ##, _, backticks, or bullet
  hyphens) — these strings are inserted directly into a Word document and markdown characters
  would appear literally and disqualify the submission.
- Do NOT include a section-heading block — the section title is rendered separately by the
  document, not generated by you.
- Where required information was not provided in the context below, use a paragraph's "missing"
  field (or a "missing" callout for a larger gap) — never invent the missing detail.

References:
- Where relevant, cite authoritative sources (program guidance documents, federal regulations,
  agency strategic plans, peer-reviewed literature) inline within paragraph text as
  (Source: [Title, URL or citation]).
- If any sources were cited, add one "references" block at the end of the section's "blocks"
  array listing them.
...
def _visual_guidance(section_id: str, section_title: str = "") -> str:
    """
    Return section-specific instructions for including figure/schedule blocks.
    Matches against both section_id and section_title (normalised to lowercase, underscores→spaces).
    Phase 4 (block-JSON migration): figures and schedules are now native block
    types in the JSON contract (FigureBlock/ScheduleBlock, models/schemas.py)
    rendered directly by utils/block_renderer.py — no more bracket markers or
    regex-parsed "Phase Name (Months X-Y)" prose.
    """
    ...
    if is_technical:
        return """
FIGURES — REQUIRED:
Include exactly two "figure" blocks among your blocks, at the point each visual would
naturally appear in a printed proposal:

1. Early in the section (right after your opening/overview paragraph block), a figure block
   whose "caption" is a short title for a process-flow diagram of YOUR technical workflow, and
   whose "altText" gives the 3-5 SPECIFIC, ACTUAL steps of that workflow — not generic labels —
   separated by " -> ", followed by one sentence of what the diagram illustrates for the reviewer.
   Example: {"type": "figure", "caption": "End-to-End Development Workflow", "altText":
   "Material Synthesis -> Nanostructure Fabrication -> Optical Characterisation -> Field
   Demonstration. Illustrates the end-to-end adaptive camouflage development workflow."}

2. Later, where a schematic or system diagram would add the most value, a figure block whose
   "caption" is a short title and whose "altText" is a specific visual description of what the
   diagram/schematic shows for this technology (materials, layers, components, data flow, etc.).

Do not also describe these figures in a separate paragraph — the figure block IS the figure;
you may still write one sentence of surrounding prose introducing or referencing it by title."""

    if is_schedule:
        return """
SCHEDULE — REQUIRED:
Include exactly one "schedule" block for the project timeline, with a real "phases" array
(3-6 phases). Each phase needs "name" (string), integer "start_month", and integer "end_month"
(1-indexed, inclusive), consistent with this grant phase's total period of performance. You may
write a short paragraph introducing the phases in prose before the schedule block, but do not
also restate the same phases as a bulleted or numbered list — the schedule block IS the
timeline."""
...
def _sanitize_json_tree(obj: Any, grant_type: str, grant_label: str) -> Any:
    """Block-JSON equivalent of _sanitize_sbir_content() — recurses through a
    parsed JSON tree (dict/list/str) and runs every string leaf through the
    same regex substitutions. Safe to apply blanket-wide: the SBIR patterns
    only match SBIR-specific phrasing, so block "type"/"kind" discriminator
    strings (e.g. "paragraph", "table", "missing") and other structural
    values never match and pass through unchanged. Must run BEFORE
    StructuredSectionContent validation so a sanitized-but-still-malformed
    value fails validation exactly like an unsanitized one would —
    substitution only ever narrows a substring, never changes JSON shape.
    """
    if isinstance(obj, str):
        return _sanitize_sbir_content(obj, grant_type, grant_label)
    if isinstance(obj, list):
        return [_sanitize_json_tree(v, grant_type, grant_label) for v in obj]
    if isinstance(obj, dict):
        return {k: _sanitize_json_tree(v, grant_type, grant_label) for k, v in obj.items()}
    return obj


def _parse_json_response(raw: str) -> Dict[str, Any]:
    """Strip markdown fences and parse JSON, with a brace-scan fallback —
    local copy of engines/supporting_documents_engine.py's helper of the
    same name (that module's established convention for this codebase)."""
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="Could not parse the AI-generated section content. Please try again.")


def _collect_missing_flags(validated: StructuredSectionContent) -> List[str]:
    """Walk the validated block tree and produce the same shape of
    compliance-flag strings the old regex `[MISSING: ...]` extraction did,
    so downstream consumers of ProposalSection.compliance_flags (UI badges,
    the future Phase 5 export QA gate) don't need to change. Sources: a
    ParagraphBlock's `missing` field (inline gap) and any CalloutBlock with
    kind=="missing" (a whole missing sub-topic)."""
    flags: List[str] = []
    for block in validated.blocks:
        if isinstance(block, ParagraphBlock) and block.missing is not None:
            flags.append(f"[MISSING: {block.missing.label}]")
        elif isinstance(block, CalloutBlock) and block.kind == "missing":
            flags.append(f"[MISSING: {block.title or block.text}]")
    return flags
...
class ProposalGeneratorEngine:
    ...
    async def generate_section(
        ...
        (CLARIVA-DOCGEN-SPEC-001), Phase 7 — agency_override is an optional
        pre-resolved AgencyProfileResolvedOut (see
        engines/agency_profile_engine.py::resolve()). Deliberately accepted
        as an already-resolved value, not a `db` session — this method is
        called concurrently via asyncio.gather() in
        routers/proposals.py::generate_all_sections(), and AsyncSession is
        not safe for concurrent use; the caller resolves the profile ONCE
        (same pattern as company_profile, loaded once and reused across the
        whole gather batch) and passes the result into every parallel call.
        None (the default) preserves the exact prior behavior — a straight
        AGENCY_GUIDANCE dict lookup, no DB involved."""
        ...
        # Phase 7: a pre-resolved AgencyProfile (admin-editable, DB-backed)
        # takes precedence over the hardcoded dict when the caller supplied one.
        if agency_override is not None and agency_override.guidance_text:
            agency_note = agency_override.guidance_text
        else:
            agency_note = (
...
IMPORTANT:
- Where required information was not provided above, use a paragraph block's "missing" field
  (or a "missing" callout for a larger gap) — never invent the missing detail, and never write
  literal "[MISSING: ...]" text inside a "text"/"caption"/"label"/item string.
- Write approximately {tw} words of rich, specific content across your paragraph/list/table blocks.
...
{_visual_guidance(section_id, section_title)}"""
        ...
        async def _call():
            ...
            data = _sanitize_json_tree(data, grant_type, grant_label)
            return data, response

        data, response = await _call()
        try:
            validated = StructuredSectionContent.model_validate(data)
        except ValidationError:
            # Same "attempt one structured regeneration if validation fails"
            # rule as supporting_documents_engine.py::generate_logic_model()
            # — never surface malformed block JSON to the user.
            data, response = await _call()
            try:
                validated = StructuredSectionContent.model_validate(data)
            except ValidationError as exc:
                _log.error("Section %s generation failed block-schema validation twice: %s", section_id, exc)
                raise HTTPException(status_code=500, detail="Could not generate valid content for this section. Please try again.")

        structured_dict = validated.model_dump()
        # flatten_section_blocks_to_text() derives the plain-text mirror kept
        # in ProposalSection.content for search/embedding/TXT-export/the
        # legacy PDF path — same "structured data is authoritative, content
        # is derived" split as the Logic Model Chart feature.
        content       = flatten_section_blocks_to_text(structured_dict)
        word_count    = len(re.findall(r"\w+", content))
        page_estimate = word_count / 250
        missing_flags = _collect_missing_flags(validated)
    ...
    async def regenerate_section(
        ...
        Not wired to any router endpoint as of Phase 4 (block-JSON
        migration) — verified via grep, nothing calls this method. Left
        emitting legacy prose rather than block JSON since there is no live
        call site to migrate or test; StructuredSectionContent-ify this at
        the same time a caller is actually wired up, not speculatively."""
        profile    = company_profile or {}
        agency     = getattr(proposal, "agency",     None) or "OTHER"
        phase      = getattr(proposal, "phase",      None) or "phase_i"
        grant_type = getattr(proposal, "grant_type", None) or "federal_other"
